...
class Line():
    ...
    def render(self, time, screen, color, fv, options):
        ...
        if 'showinfo' in options and self.id == int(options['showinfo']):
            lineInfo = font.render(f'ID: {int(self.id)}', False, (0, 0, 0))
            screen.blit(lineInfo, (10, 50))
            lineInfo = font.render(f'Time: {int(time)}', False, (0, 0, 0))
            screen.blit(lineInfo, (10, 65))
            lineInfo = font.render(f'FloorPos: {int(self.floorPosition)}', False, (0, 0, 0))
            screen.blit(lineInfo, (10, 80))
            lineInfo = font.render(f'Rotation: {int(self.deg)} deg', False, (0, 0, 0))
            screen.blit(lineInfo, (10, 95))
            lineInfo = font.render(f'Alpha: {int(alpha)}', False, (0, 0, 0))
