...
class RPE:
    ...
    def __init__(self, chart):
        RPE.BPMlist = BPMList(chart['BPMList'])
        ...
        self.lines = []
        self.parseLine()
        self.all_notes = [note for line in self.lines for note in line.noteList if not note.isFake]
        self.notes = len(self.all_notes)
        self.all_notes.sort(key=lambda x: x.endTime)
        self.highlight()
        self.jm = RPEJudgeManager(self.all_notes)
    def __repr__(self):
        return f'RPE Chart[Notes={self.notes}, Lines={len(self.lines)}]'
    def parseLine(self):
        for i, line in enumerate(self.judgeLineList):
            self.lines.append(Line(line, self, i))
    def highlight(self):
    ...
    def render(self, time, screen, options):
        notelist = filter(lambda x: time <= RPE.BPMlist.beat2sec(x.endTime, x.line.bpmfactor), self.all_notes)
        for i in notelist:
...
class Line:
    def __init__(self, line, chart, id):
        ...
        self.eventLayers = line['eventLayers']
        self.events = self.eventLayers[0]
        for i, j in enumerate(self.eventLayers):
            if i == 0 or not j:
                continue
            for k, l in j.items():
                self.events[k] += l
        ...
        self.scaleYEvents = []
        self.textEvents = []'''
        self.bpmfactor = line.get('bpmfactor', 1)
        self.parseEvent()
        self.parseNote()
    ...
    def __repr__(self):
        return f'JudgeLine[id={self.id}, Notes={len(self.noteList)}, Events={len(self.events)}]'
    def parseNote(self):
        id = 0
        ...
        for i in speedEvents:
            ...
            if i.startTime != i.endTime and i.start != i.end:
                for j in range(int(i.startTime), int(i.endTime)):
                    events.append({'time': j + 1, 'value': Easing(i).getValue(j + 1)})
            if i.start == i.end:
    ...
    def parseEvent(self):
        ...
        for i, j in enumerate(self.speedEvents):
            ...
            val = j['value'] * 11 / 45
            pos = floorPos
            floorPos += RPE.BPMlist.beat2sec((end - start), self.bpmfactor) * val
    ...
    def render(self, time, screen):
        time = RPE.BPMlist.sec2beat(time, self.bpmfactor)
        ...
        for i in self.speedEvents:
            ...
            if time < i['startTime']:
                break
            self.floorPosition = i['floorPosition'] + i['value'] * RPE.BPMlist.beat2sec(time - i['startTime'], self.bpmfactor)
            break
...
class Easing:
    ...
    @classmethod
    def clear_cache(cls):
        cls._bezier_cache.clear()
        cls._cache_stats = {'hits': 0, 'misses': 0}
    @classmethod
    def cache_info(cls):
        total = cls._cache_stats['hits'] + cls._cache_stats['misses']
        ratio = cls._cache_stats['hits'] / total if total > 0 else 0
        return f"Bezier Cache - Hits: {cls._cache_stats['hits']}, Misses: {cls._cache_stats['misses']}, Hit Ratio: {ratio:.1%}"
    ...
    def bezier(self, time):
        cache_key = (time, self.startTime, self.endTime, tuple(self.points), self.start, self.end)
        if cache_key in Easing._bezier_cache:
            Easing._cache_stats['hits'] += 1
            return Easing._bezier_cache[cache_key]
        Easing._cache_stats['misses'] += 1
        ...
        x = bisect(eq, 0, 1)
        result = bezierY(x, p1, p2, p3, p4) * (self.end - self.start) + self.start
        Easing._bezier_cache[cache_key] = result
        return result
class Event:
    def __init__(self, event, line):
        ...
        self.start = event['start']
        self.end = event['end']
        self.startTime = toBeat(event['startTime'])
        self.endTime = toBeat(event['endTime'])
    def __repr__(self):
        return 'Event[Time={:.2f}-{:.2f}]'.format(self.startTime, self.endTime)
class Note:
    def __init__(self, note, line, id):
        ...
        self.endTime = toBeat(note['endTime'])
        self.startTime = toBeat(note['startTime'])
        ...
        self.scored = False
        self.realTime = RPE.BPMlist.beat2sec(self.startTime, line.bpmfactor)
        self.hl = False
        self.hit = False
        if DEBUG_NOTE:
            self.id_text = util.font(20).render(f'{self.line.id},{self.id}', False, (255, 0, 0))
    def __repr__(self):
        return f'{[None, "Tap", "Hold", "Flick", "Drag"][self.type]}Note[id={self.line.id},{self.id}]'
    def addFloorPosition(self):
        ...
        self.floorPosition = v1 + v2 * RPE.BPMlist.beat2sec(v3, self.line.bpmfactor)
        if self.type == 3:
            self.speed *= v2
    def render(self, time, screen):
        realTime = RPE.BPMlist.sec2beat(time, self.line.bpmfactor)
        if self.scored or self.line.alpha < 0:
            return
        if self.realTime - realTime > self.visibleTime or realTime - RPE.BPMlist.sec2beat(self.endTime, self.line.bpmfactor) > self.visibleTime:
            return
        ...
        if self.type != 3:
            ...
            if time <= self.startTime:
                yDistEnd = yDist + self.speed * RPE.BPMlist.beat2sec(self.endTime - self.startTime, self.line.bpmfactor)
            elif self.startTime < time <= self.endTime:
                yDistEnd = self.speed * RPE.BPMlist.beat2sec(self.endTime - time, self.line.bpmfactor)
            else:
                yDistEnd = 0
...
class BPMList:
    ...
    def parse(self, bpmlist):
        for i in bpmlist:
            i['startTime'] = max(0, toBeat(i['startTime']))
            i['invbpm'] = 60 / i['bpm']
            self.list.append(i)
    def sec2beat(self, t, bpmfactor):
        beat = 0
        for i, e in enumerate(self.list):
            if (e['invbpm'], bpmfactor) in BPMList._bpm_cache:
                invbpmv = BPMList._bpm_cache[(e['invbpm'], bpmfactor)]
            else:
                invbpmv = e['invbpm'] * bpmfactor
                BPMList._bpm_cache[(e['invbpm'], bpmfactor)] = invbpmv
            if i != len(self.list) - 1:
                et_beat = self.list[i + 1]['startTime'] - e['startTime']
                et_sec = et_beat * invbpmv
                if t >= et_sec:
                    beat += et_beat
                    t -= et_sec
                else:
...
class RPEJudgeManager():
    ...
    def check(self, time, screen):
        ...
        for i in self.note_list:
            beat = RPE.BPMlist.sec2beat(time, i.line.bpmfactor)
            if i.endTime <= beat:
                self.combo += 1
                if i.type != 3:
                    hit(screen, *i.pos, i.deg, i.type)
                i.scored = 1
